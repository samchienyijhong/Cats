...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valid_paragraph = [p for p in paragraphs if select(p)]
...
def about(topic):
    ...
    #     return False
    # return select
    return lambda p: any([word in topic for word in split(remove_punctuation(lower(p)))])
    # END PROBLEM 2
...
def fastest_words(game):
    ...
    for word_indice in word_indices:
        ...
        for player_indice in player_indices:
            if time(game, player_indice, word_indice) < fastest_time:
                fastest_time = time(game, player_indice, word_indice)
                fastest_player = player_indice
        words[fastest_player].append(word_at(game, word_indice))
    return words
    # END PROBLEM 10
